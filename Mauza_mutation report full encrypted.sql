...
WITH RawData AS (
    SELECT 
        ...
        -- 2. SMART NAME DECRYPTION (Hybrid Approach)
        -- Checks History -> Reference -> Temp
        -- Tries to DECRYPT first. If that fails, shows Raw Text.
        COALESCE(
            -- CHECK 1: HistoryPerson (Try Decrypt -> Fallback to Raw)
            COALESCE(CAST(decryptbykeyautocert(cert_id('Usercert'), NULL, HP.first_name) AS NVARCHAR(MAX)), CAST(HP.first_name AS NVARCHAR(MAX))) + ' ' + 
            ISNULL(COALESCE(CAST(decryptbykeyautocert(cert_id('Usercert'), NULL, HP.last_name) AS NVARCHAR(MAX)), CAST(HP.last_name AS NVARCHAR(MAX))), ''),

            -- CHECK 2: Reference Person
            COALESCE(CAST(decryptbykeyautocert(cert_id('Usercert'), NULL, RP.first_name) AS NVARCHAR(MAX)), CAST(RP.first_name AS NVARCHAR(MAX))) + ' ' + 
            ISNULL(COALESCE(CAST(decryptbykeyautocert(cert_id('Usercert'), NULL, RP.last_name) AS NVARCHAR(MAX)), CAST(RP.last_name AS NVARCHAR(MAX))), ''),

            -- CHECK 3: Temporary Person
            COALESCE(CAST(decryptbykeyautocert(cert_id('Usercert'), NULL, TP.first_name) AS NVARCHAR(MAX)), CAST(TP.first_name AS NVARCHAR(MAX))) + ' ' + 
            ISNULL(COALESCE(CAST(decryptbykeyautocert(cert_id('Usercert'), NULL, TP.last_name) AS NVARCHAR(MAX)), CAST(TP.last_name AS NVARCHAR(MAX))), ''),
